...
export default class NativeCategoryCommunity extends Component {
  ...
  @action
  categoryChanged() {
    this.community = this.args.community || null;
  ...
  async loadCommunity() {
    ...
    try {
      ...
      if (this.args.category?.id === categoryId) {
  ...
  async updateMembership(type) {
    ...
    const categoryId = this.args.category?.id;
    const communitySlug = this.community.slug;
    this.membershipBusy = true;
    this.errorMessage = null;

    try {
      const response = await ajax(
        `/community-platform/communities/${encodeURIComponent(communitySlug)}/join.json`,
        { type }
      );

      if (
        this.args.category?.id === categoryId &&
        this.community?.slug === communitySlug
      ) {
        this.community = response.community;
      }
    } catch {
  ...
  <template>
    <div
      class="dcp-native-community-mount"
      {{didUpdate this.categoryChanged @category.id}}
    >
      {{#if this.community}}
